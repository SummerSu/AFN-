...
@property (nonatomic, assign) long autoid;
...
@property (nonatomic, assign) BOOL provideServiceDiscount;  //是否有优惠券
@property (nonatomic, assign) BOOL provideServiceNo;        //没有提供服务
@property (nonatomic, assign) BOOL provideServiceOrder;     //订座
@property (nonatomic, assign) BOOL provideServiceTakeout;   //外卖
...
@property (nonatomic, assign) long uid;                     //商户id
@property (nonatomic, assign) long weight;


+ (ShopListModel *)modelWithDic:(NSDictionary *)dic;

+ (void)getShopListDatasWithUrl:(NSString *)url parameters:(NSDictionary *)parameters andBlock:(void (^)(NSArray *datas, NSError *error))block;
